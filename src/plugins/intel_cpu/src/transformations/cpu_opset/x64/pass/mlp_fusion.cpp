...
ov::intel_cpu::MLPFusion::MLPFusion() {
    ...
    auto gate_proj_weight_deq = makePattern<op::v1::Multiply>({gate_proj_weight_f32, gate_proj_weight_scales_per_OC},
    ...
    auto up_proj_weight_deq = makePattern<op::v1::Multiply>({up_proj_weight_f32, up_proj_weight_scales_per_OC},
                                                            {{"auto_broadcast", "numpy"}});
    ...
    auto down_proj_weight_deq = makePattern<op::v1::Multiply>({down_proj_weight_f32, down_proj_weight_scales_per_OC},
                                                              {{"auto_broadcast", "numpy"}});
    ...
    auto gate_up_proj_weight_deq =
        makePattern<op::v1::Multiply>({gate_up_proj_weight_cvt_f32, gate_up_proj_weight_scales_per_OC},
                                      {{"auto_broadcast", "numpy"}});
    ...
    auto gate_up_split_lengths = makeConst(ov::element::i32,
                                           ...
                                           nullptr);
    auto gate_up_proj_split = makePattern<op::v1::VariadicSplit>({gate_up_proj, -1, gate_up_split_lengths});
    ...
    auto mlp_silu_gate = makePattern<op::v4::Swish>({mlp_gate_proj | gate_up_proj_split->output(0)});
    auto mlp_gelu_gate = makePattern<op::v7::Gelu>({mlp_gate_proj | gate_up_proj_split->output(0)});
    auto mlp_up_proj =
        makePattern<op::v0::MatMul>({input, up_proj_weight | up_proj_weight_compressed | up_proj_weight_deq},
                                    {{"transpose_a", false}, {"transpose_b", true}});

    auto mlp_gated_up =
        makePattern<op::v1::Multiply>({mlp_silu_gate | mlp_gelu_gate, mlp_up_proj | gate_up_proj_split->output(1)},
                                      {{"auto_broadcast", "numpy"}});
