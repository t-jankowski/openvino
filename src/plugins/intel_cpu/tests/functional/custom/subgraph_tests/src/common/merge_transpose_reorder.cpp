...
static std::shared_ptr<ov::Node> make_layer_with_bias(Args&&... args) {
    ...
    const auto bias = std::make_shared<ov::op::v1::Add>(node, bias_const);
    return bias;
}
...
protected:
    void SetUp() override {
        ...
        const auto param = std::make_shared<ov::op::v0::Parameter>(precision, inputDynamicShapes[0]);
        const auto reshape_const_1 = ov::op::v0::Constant::create(shapeof_subgraph_prc, {3}, {0, 0, -1});
        const auto reshape_1 = std::make_shared<ov::op::v1::Reshape>(param, reshape_const_1, true);

        const auto transpose_const_1 = ov::op::v0::Constant::create(shapeof_subgraph_prc, {3}, {0, 2, 1});
        const auto transpose_1 = std::make_shared<ov::op::v1::Transpose>(reshape_1, transpose_const_1);
        ...
        const auto fc_1 = make_layer_with_bias<ov::op::v0::MatMul>(transpose_1, fc_weights_1, false, true);

        const auto transpose_const_2 = ov::op::v0::Constant::create(shapeof_subgraph_prc, {3}, {0, 2, 1});
        const auto transpose_2 = std::make_shared<ov::op::v1::Transpose>(fc_1, transpose_const_2);
        const auto spatial_dims = ov::op::util::node_to_get_shape_value_of_indices_from_shape_source(param, {2, 3}, {}, shapeof_subgraph_prc);
        const auto unchangable_dims = ov::op::v0::Constant::create(shapeof_subgraph_prc, {2}, {0, 0});
        const auto reshape_const_2 = ov::op::util::make_try_fold<ov::op::v0::Concat>(ov::OutputVector{unchangable_dims, spatial_dims}, 0);
        const auto reshape_2 = std::make_shared<ov::op::v1::Reshape>(transpose_2, reshape_const_2, true);

        const auto conv_weights = ov::test::utils::make_constant(precision, ov::Shape{fc_out_channels, 1, 1, 3, 3});
        const auto conv_with_bias = make_layer_with_bias<ov::op::v1::GroupConvolution>(reshape_2,
                                                                              conv_weights,
                                                                              ov::Strides{1, 1},
        ...
        const auto fused_spatial_dims = ov::op::util::make_try_fold<ov::op::v1::Multiply>(dim_h, dim_w);
        const auto reshape_const_3 = ov::op::util::make_try_fold<ov::op::v0::Concat>(ov::OutputVector{unchangable_dims, fused_spatial_dims}, 0);
        const auto reshape_3 = std::make_shared<ov::op::v1::Reshape>(conv_with_bias, reshape_const_3, true);
        const auto transpose_const_3 = ov::op::v0::Constant::create(shapeof_subgraph_prc, {3}, {0, 2, 1});
        const auto transpose_3 = std::make_shared<ov::op::v1::Transpose>(reshape_3, transpose_const_3);

        const auto fc_weights_2 = ov::test::utils::make_constant(precision, ov::Shape{channels, fc_out_channels});
        const auto fc_2 = make_layer_with_bias<ov::op::v0::MatMul>(transpose_3, fc_weights_2, false, true);
