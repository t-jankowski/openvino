...
template <class T>
static std::shared_ptr<ov::Model> createInitGraph(std::shared_ptr<ov::op::v0::Parameter> param) {
...
static std::shared_ptr<ov::Model> createRefGraph(ov::Shape param_shape) {
        ...
        for (auto axis : axes) {
            auto reduction_axis = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{}, {axis});
...
static bool registerAndRunReducePass(std::shared_ptr<ov::Model> model) {
    ...
    if (std::is_same<T, ov::op::v1::ReduceMin>::value) {
        manager.register_pass<ConvertReduceMin>();
    } else if (std::is_same<T, ov::op::v1::ReduceMax>::value) {
        manager.register_pass<ConvertReduceMax>();
    } else if (std::is_same<T, ov::op::v1::ReduceSum>::value) {
        manager.register_pass<ConvertReduceSum>();
    } else if (std::is_same<T, ov::op::v1::ReduceProd>::value) {
...
TYPED_TEST_P(ConvertReduceMultiAxisTest, CheckConvertReduceTransformationIsApplied) {
    auto param = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, static_param_shape);
...
TYPED_TEST_P(ConvertReduceMultiAxisTest, CheckConvertReduceTransformationIsNotAppliedForDynaimcShapes) {
    auto param = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, dynamic_param_shape);
    auto model = createInitGraph<TypeParam>(param);
...
using reduceTypes = ::testing::Types<ov::op::v1::ReduceMin,
                                     ov::op::v1::ReduceMax,
                                     ov::op::v1::ReduceSum,
                                     ov::op::v1::ReduceProd>;
