...
static std::shared_ptr<ov::Model> createInitGraph(ov::Shape param_shape, ov::Shape weights_shape) {
        ...
        auto param = std::make_shared<ov::op::v0::Parameter>(type, param_shape);
...
static std::shared_ptr<ov::Model> createTransformedGraph(ov::Shape param_shape, ov::Shape weights_shape) {
        ...
        auto reshape = std::make_shared<ov::op::v0::Squeeze>(conv2d,
...
TEST(TransformationTests, CheckConvertConv1DIsAppliedFor1DShapes) {
    ...
    {
        model = createInitGraph<ov::op::v1::Convolution>(ov::Shape{2, 64, 7}, ov::Shape{ 30, 64, 1 });
        ov::pass::Manager manager;
        manager.register_pass<ConvertConv1D>();
        manager.run_passes(model);
    }
    {
        model_ref = createTransformedGraph<ov::op::v1::Convolution>(ov::Shape{2, 64, 7}, ov::Shape{30, 64, 1});
    }
...
TEST(TransformationTests, CheckConvertConv1DIsNotAppliedFor2DShapes) {
    std::shared_ptr<ov::Model> model(nullptr), model_ref(nullptr);
    {
        model = createInitGraph<ov::op::v1::Convolution>(ov::Shape{2, 64, 7, 1}, ov::Shape{30, 64, 1, 1});
        ov::pass::Manager manager;
        manager.register_pass<ConvertConv1D>();
        manager.run_passes(model);
    }
    {
        model_ref = createInitGraph<ov::op::v1::Convolution>(ov::Shape{2, 64, 7, 1}, ov::Shape{30, 64, 1, 1});
    }
    auto res = compare_functions(model, model_ref);
    ASSERT_TRUE(res.first) << res.second;
}
...
TEST(TransformationTests, CheckConvertGroupConv1DIsAppliedFor1dShapes) {
    std::shared_ptr<ov::Model> model(nullptr), model_ref(nullptr);
    {
        model = createInitGraph<ov::op::v1::GroupConvolution>(ov::Shape{1, 12, 64}, ov::Shape{4, 1, 3, 5});
        ov::pass::Manager manager;
        manager.register_pass<ConvertGroupConv1D>();
        manager.run_passes(model);
    }
    {
        model_ref = createTransformedGraph<ov::op::v1::GroupConvolution>(ov::Shape{1, 12, 64}, ov::Shape{4, 1, 3, 5});
    }
    auto res = compare_functions(model, model_ref);
    ASSERT_TRUE(res.first) << res.second;
}
...
TEST(TransformationTests, CheckConvertGroupConv1DIsNotAppliedFor2DShapes) {
    std::shared_ptr<ov::Model> model(nullptr), model_ref(nullptr);
    {
        model = createInitGraph<ov::op::v1::GroupConvolution>(ov::Shape{1, 12, 64, 1}, ov::Shape{4, 1, 3, 5, 1});
        ov::pass::Manager manager;
        manager.register_pass<ConvertGroupConv1D>();
        manager.run_passes(model);
    }
    {
        model_ref = createInitGraph<ov::op::v1::GroupConvolution>(ov::Shape{1, 12, 64, 1}, ov::Shape{4, 1, 3, 5, 1});
    }
    auto res = compare_functions(model, model_ref);
    ASSERT_TRUE(res.first) << res.second;
}
