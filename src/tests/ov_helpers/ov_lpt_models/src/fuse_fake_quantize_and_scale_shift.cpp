...
std::shared_ptr<ov::Model> FuseFakeQuantizeAndScaleShiftFunction::getOriginal(
    ...
    const std::shared_ptr<Node> multiply = std::make_shared<ov::op::v1::Multiply>(
        fakeQuantize,
        std::make_shared<ov::op::v0::Constant>(precision, Shape{ 1, 1, 1, 1 }, std::vector<float>({ 150 })));

    const std::shared_ptr<Node> add = std::make_shared<ov::op::v1::Add>(
        multiply,
        std::make_shared<ov::op::v0::Constant>(precision, Shape{ 1, 1, 1, 1 }, std::vector<float>({ 127.5 })));
