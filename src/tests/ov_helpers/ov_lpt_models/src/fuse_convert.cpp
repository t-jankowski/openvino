...
std::shared_ptr<ov::Model> FuseConvertFunction::get(
    ...
    if (constInput) {
        parent = std::make_shared<ov::op::v0::Constant>(inputPrecision, inputShape.to_shape(), std::vector<float>{ 128.f });
    } else {
...
std::shared_ptr<ov::Model> FuseConvertFunction::getWithFQ(
    ...
    std::shared_ptr<ov::op::v0::Parameter> input2 = std::make_shared<ov::op::v0::Parameter>(
            inputPrecision,
            inputShape);
    ...
    const auto power = std::make_shared<ov::op::v1::Power>(
        fakeQuantizeOnActivations,
        std::make_shared<ov::op::v0::Constant>(inputPrecision, Shape{}, std::vector<float>{2.f}));
