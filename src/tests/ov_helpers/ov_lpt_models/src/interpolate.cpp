...
std::shared_ptr<ov::Model> InterpolateFunction::getOriginal(
    ...
    const auto outShape = std::make_shared<ov::op::v0::Constant>(ov::element::i64, ov::Shape{ outputShape.size() }, outputShape);
    const auto interpolate = std::make_shared<ov::op::v0::Interpolate>(dequantizationOp, outShape, interpAttrs);
...
std::shared_ptr<ov::Model> InterpolateFunction::getOriginal(
    ...
    const auto outShape = std::make_shared<ov::op::v0::Constant>(ov::element::i64, ov::Shape{ outputShape.size() }, outputShape);
    const auto interpolate = std::make_shared<ov::op::v0::Interpolate>(fakeQuantizeOnActivations, outShape, interpAttrs);
...
std::shared_ptr<ov::Model> InterpolateFunction::getReference(
    ...
    const auto outShape = std::make_shared<ov::op::v0::Constant>(ov::element::i64, ov::Shape{ outputShape.size() }, outputShape);
    const auto interpolate = std::make_shared<ov::op::v0::Interpolate>(quantizationOpBefore, outShape, interpAttrs);
...
std::shared_ptr<ov::Model> InterpolateFunction::getOriginal(
    ...
    const auto dequantizationOp = makeDequantization(input, dequantization);
    const auto outShape = std::make_shared<ov::op::v0::Constant>(ov::element::i64, ov::Shape{ outputShape.size() }, outputShape);
    const auto scales = std::make_shared<ov::op::v0::Constant>(ov::element::f32, ov::Shape{ scalesShape.size() }, scalesShape);
