...
std::shared_ptr<ov::Model> SplitFunction::getOriginal(
    ...
    const auto split = std::make_shared<ov::op::v1::Split>(dequantizationOp, constant, numSplits);
...
std::shared_ptr<ov::Model> SplitFunction::getOriginal(
    ...
    auto constant = std::make_shared<ov::op::v0::Constant>(ov::element::i64, Shape{ }, splitedAxis);
    const std::shared_ptr<ov::op::v1::Split> split = std::make_shared<ov::op::v1::Split>(fq, constant, numSplit);
...
std::shared_ptr<ov::Model> SplitFunction::getReference(
    ...
    const auto constant = std::make_shared<ov::op::v0::Constant>(ov::element::i64, Shape{ }, splitedAxis);
    const auto split = std::make_shared<ov::op::v1::Split>(deqBefore, constant, numSplit);
    ...
    for (size_t i = 0; i < numSplit; ++i) {
        if (!dequantizationAfter.empty()) {
            ...
            results.push_back(std::make_shared<ov::op::v0::Result>(makeDequantization(split->output(i), dequantizationAfter[i])));
        } else {
            results.push_back(std::make_shared<ov::op::v0::Result>(split->output(i)));
        }
    }
