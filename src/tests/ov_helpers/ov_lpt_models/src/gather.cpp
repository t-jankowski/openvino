...
std::shared_ptr<ov::Model> GatherFunction::getOriginal(
    ...
    const auto indicesNode = std::make_shared<ov::op::v0::Constant>(
        ov::element::i64,
    ...
    if (opset_version == 7) {
        gather = std::make_shared<ov::op::v7::Gather>(dequantizationOp, indicesNode, axisNode, batch_dims);
    } else if (opset_version == 8) {
        gather = std::make_shared<ov::op::v8::Gather>(dequantizationOp, indicesNode, axisNode, batch_dims);
    } else if (opset_version == 1) {
        gather = std::make_shared<ov::op::v1::Gather>(dequantizationOp, indicesNode, axisNode);
    } else {
...
std::shared_ptr<ov::Model> GatherFunction::getOriginal(
    ...
    std::shared_ptr<Node> gather;
    if (opset_version == 7) {
        gather = std::make_shared<ov::op::v7::Gather>(quantizationOp, indicesNode, axisNode, batch_dims);
    } else if (opset_version == 8) {
        gather = std::make_shared<ov::op::v8::Gather>(quantizationOp, indicesNode, axisNode, batch_dims);
    } else if (opset_version == 1) {
        gather = std::make_shared<ov::op::v1::Gather>(quantizationOp, indicesNode, axisNode);
    } else {
        throw std::runtime_error("Unknown opset version");
    }
...
std::shared_ptr<ov::Model> GatherFunction::getReference(
    ...
    std::shared_ptr<Node> gather;
    if (opset_version == 7) {
        gather = std::make_shared<ov::op::v7::Gather>(quantizationOpBefore, indicesNode, axisNode, batch_dims);
    } else if (opset_version == 8) {
        gather = std::make_shared<ov::op::v8::Gather>(quantizationOpBefore, indicesNode, axisNode, batch_dims);
    } else if (opset_version == 1) {
        gather = std::make_shared<ov::op::v1::Gather>(quantizationOpBefore, indicesNode, axisNode);
    } else {
        throw std::runtime_error("Unknown opset version");
    }
