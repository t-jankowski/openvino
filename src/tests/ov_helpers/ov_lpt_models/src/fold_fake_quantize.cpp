...
std::shared_ptr<ov::Model> FoldFakeQuantizeFunction::getOriginal(
    ...
    const auto constant = std::make_shared<ov::op::v0::Constant>(precision, constShape, constValues);
...
std::shared_ptr<ov::Model> FoldFakeQuantizeFunction::getReference(
    ...
    const ov::Shape& constShape,
    const std::vector<float>& constValues) {
    const std::shared_ptr<Node> constant = std::make_shared<ov::op::v0::Constant>(precision, constShape, constValues);
