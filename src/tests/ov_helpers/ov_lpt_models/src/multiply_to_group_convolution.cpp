...
std::shared_ptr<ov::Model> MultiplyToGroupConvolutionFunction::getOriginal(
    ...
    if (haveMultiplyWithNoConstBeforeDequantization) {
        secondInput = std::make_shared<ov::op::v0::Parameter>(precisionBeforeDequantization, inputShape);
        parent = std::make_shared<ov::op::v1::Multiply>(input, secondInput);
    }
...
std::shared_ptr<ov::Model> MultiplyToGroupConvolutionFunction::getOriginal(
    ...
    const auto multiply = std::make_shared<ov::op::v1::Multiply>(
        maxPool,
        std::make_shared<ov::op::v0::Constant>(constant.outPrecision, constant.shape, constant.values));
    multiply->set_friendly_name("output");

    ov::ResultVector results = parentHasOneConsumer ?
        ov::ResultVector{std::make_shared<ov::op::v0::Result>(multiply)} :
        ov::ResultVector{std::make_shared<ov::op::v0::Result>(maxPool), std::make_shared<ov::op::v0::Result>(multiply)};
...
std::shared_ptr<ov::Model> MultiplyToGroupConvolutionFunction::getReference(
    ...
    const auto gconv = std::make_shared<ov::op::TypeRelaxed<ov::op::v1::GroupConvolution>>(
    ...
    if (biases) {
        lastNode = std::make_shared<ov::op::v1::Add>(gconv, biases);
    }
