...
TEST_F(TransformationTestsF, NormalizeL2DecomositionFusionWithMax) {
    const float eps_value = 0.000099f;
    {
        ...
        auto axes_const = op::v0::Constant::create(element::i64, Shape{2}, {1, 2});
...

    {
        ...
        auto axes_const = op::v0::Constant::create(element::i64, Shape{2}, {1, 2});
        auto reduce_sum = std::make_shared<op::v1::ReduceSum>(pow, axes_const, true);
...
TEST_F(TransformationTestsF, NormalizeL2DecomositionFusionWithAdd) {
...

    {
        ...
        auto reduce_sum = std::make_shared<op::v1::ReduceSum>(pow, axes_const, true);
        auto eps_const = op::v0::Constant::create(element::f16, Shape{}, {eps_value});
        auto max = std::make_shared<op::v1::Add>(reduce_sum, eps_const);
        auto sqrt = std::make_shared<op::v0::Sqrt>(max);
        auto divide = std::make_shared<op::v1::Divide>(input, sqrt);
