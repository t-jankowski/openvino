...
std::shared_ptr<ov::Model> create_v15_model(const Reduction reduction_type) {
    const auto data = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{1000, 256, 10, 15});
    const auto indices = std::make_shared<ov::op::v0::Parameter>(ov::element::i32, ov::Shape{25, 125, 3});
    const auto updates = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{25, 125, 15});
    const auto scatter_nd = std::make_shared<ov::op::v15::ScatterNDUpdate>(data, indices, updates, reduction_type);
...
std::shared_ptr<ov::Model> create_v3_model() {
    ...
    const auto indices = std::make_shared<ov::op::v0::Parameter>(ov::element::i32, ov::Shape{25, 125, 3});
    const auto updates = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{25, 125, 15});
    const auto scatter_nd = std::make_shared<ov::op::v3::ScatterNDUpdate>(data, indices, updates);
    scatter_nd->set_friendly_name("scatter_nd15");
