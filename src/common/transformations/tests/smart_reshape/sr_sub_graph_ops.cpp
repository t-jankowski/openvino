...
TEST(SmartReshapeTests, TensorIteratorStaticParameters) {
    std::shared_ptr<ov::Model> f(nullptr);
    {
        ...
        auto body_condition = std::make_shared<op::v0::Constant>(element::boolean, Shape{}, true);

        // Body
        auto sum = std::make_shared<op::v1::Add>(Xi, Yi);
        auto Zo = std::make_shared<op::v1::Multiply>(sum, M_body);
...
TEST(SmartReshapeTests, LoopStaticParameters) {
    std::shared_ptr<ov::Model> f(nullptr);
    {
        ...
        auto trip_count = std::make_shared<op::v0::Constant>(element::i64, Shape{}, 10);
...
TEST(SmartReshapeTests, LoopParentParametersUsedInBody) {
    std::shared_ptr<ov::Model> f(nullptr);
    {
        ...
        auto X = std::make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic());
        auto Y = std::make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic());
        auto add_Y = std::make_shared<op::v1::Add>(
            Y,
            std::make_shared<op::v0::Constant>(element::f32, Shape{}, std::vector<float>{0.f}));
        auto M = std::make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic());
