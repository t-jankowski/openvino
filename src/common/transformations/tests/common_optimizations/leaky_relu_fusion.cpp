...
TEST_F(TransformationTestsF, LeakyReluFusionConstantGreaterThanOne) {
    {
        auto data = std::make_shared<op::v0::Parameter>(element::f32, Shape{2, 2});
        auto alpha = op::v0::Constant::create(element::f32, Shape{1}, {1.1});
        auto multiply = std::make_shared<op::v1::Multiply>(data, alpha);
        auto max = std::make_shared<op::v1::Maximum>(data, multiply);
        model = std::make_shared<Model>(NodeVector{max}, ParameterVector{data});
...
TEST_F(TransformationTestsF, LeakyReluFusionConstantAlphaOnFirstInput) {
    {
        auto alpha = op::v0::Constant::create(element::f32, Shape{1}, {0.1});
        auto data = std::make_shared<op::v0::Parameter>(element::f32, Shape{2, 2});
        auto multiply = std::make_shared<op::v1::Multiply>(alpha, data);
        auto max = std::make_shared<op::v1::Maximum>(multiply, data);
        model = std::make_shared<Model>(NodeVector{max}, ParameterVector{data});
