...
ov::snippets::pass::ExtractReshapesFromMHA::ExtractReshapesFromMHA() {
    ...
    auto matmul_m = pattern::wrap_type<ov::op::v0::MatMul>(static_shape_single_consumer);
    auto reshape_1_m = pattern::wrap_type<ov::op::v1::Reshape>({matmul_m, pattern::wrap_type<ov::op::v0::Constant>()}, static_shape_single_consumer);
    ...
    auto add_1_m = pattern::wrap_type<ov::op::v1::Add>({reshape_1_m, sparse_input_1_m}, static_shape_single_consumer);
    auto add_2_m = pattern::wrap_type<ov::op::v1::Add>({add_1_m, sparse_input_2_m}, static_shape_single_consumer);
    auto reshape_2_m = pattern::wrap_type<ov::op::v1::Reshape>({add_2_m, pattern::wrap_type<ov::op::v0::Constant>()}, pattern::has_static_shape());
    ...
    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](pattern::Matcher& m) {
        ...
        const auto matmul_node = ov::as_type_ptr<ov::op::v0::MatMul>(matmul.get_node_shared_ptr());
        ...
        const auto extracted_add = std::make_shared<ov::op::v1::Add>(sparse_input_1, sparse_input_2);
        const auto target_shape = ov::op::v0::Constant::create(ov::element::i32, {matmul_shape.size()}, matmul_shape);
        const auto extracted_reshape = std::make_shared<ov::op::v1::Reshape>(extracted_add, target_shape, true);
        const auto new_add = std::make_shared<ov::op::v1::Add>(matmul, extracted_reshape);
