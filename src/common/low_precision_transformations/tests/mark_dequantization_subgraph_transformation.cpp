...
static std::shared_ptr<ov::Node> make_branch(const std::shared_ptr<ov::op::v0::Convert> zp_convert,
        ...
        // const data path
        const auto lp_const = std::make_shared<op::v0::Constant>(element::i8, lp_const_shape, 1);
        const auto data_convert = std::make_shared<op::v0::Convert>(lp_const, element::f32);

        const auto zp_target_shape = std::make_shared<op::v0::Constant>(ov::element::i64, ov::Shape{sub_mul_shape.size()}, sub_mul_shape);
        const auto zp_reshape = std::make_shared<op::v1::Reshape>(zp_convert, zp_target_shape, false);

        const auto subtract = std::make_shared<op::v1::Subtract>(data_convert, zp_reshape);

        // scale
        std::shared_ptr<ov::Node> scale_const;
        std::shared_ptr<ov::Node> scale_reshape;
        if (ref_model) {
            scale_const = std::make_shared<op::v0::Constant>(element::f32, ov::Shape{sub_mul_shape}, 1);
            scale_reshape = scale_const;
        } else {
            scale_const = std::make_shared<op::v0::Constant>(element::f32, ov::Shape{64}, 1);
            auto scale_target_shape = std::make_shared<op::v0::Constant>(ov::element::i64, ov::Shape{sub_mul_shape.size()}, sub_mul_shape);
            scale_reshape = std::make_shared<op::v1::Reshape>(scale_const, scale_target_shape, false);
        }

        const auto multiply = std::make_shared<op::v1::Multiply>(subtract, scale_reshape);
...
TEST_F(TransformationTestsF, KeepConstPrecision2BranchesDiffShapes) {
    {
        ...
        // zero points
        const auto zp_const = std::make_shared<op::v0::Constant>(element::i8, ov::Shape{64}, 1);
        const auto zp_convert = std::make_shared<op::v0::Convert>(zp_const, element::f32);

        const auto right_branch = make_branch(zp_convert, {128,64,2,2}, {1, 64, 1, 1});
        const auto result_right = std::make_shared<op::v0::Result>(right_branch);

        const auto left_branch = make_branch(zp_convert, {64,3,3,3}, {64, 1, 1, 1});
        const auto result_left = std::make_shared<op::v0::Result>(left_branch);
...
static std::shared_ptr<ov::Node> make_branch_same(const std::shared_ptr<ov::op::v0::Convert> zp_convert) {
    // const data
    const auto lp_const = std::make_shared<op::v0::Constant>(element::i8, ov::Shape{64, 3, 3, 3}, 1);
    const auto data_convert = std::make_shared<op::v0::Convert>(lp_const, element::f32);

    const auto subtract = std::make_shared<op::v1::Subtract>(data_convert, zp_convert);

    // scale
    const auto scale_const = std::make_shared<op::v0::Constant>(element::f32, ov::Shape{64, 1, 1, 1}, 1);
    const auto multiply = std::make_shared<op::v1::Multiply>(subtract, scale_const);
...
TEST_F(TransformationTestsF, KeepConstPrecision2BranchesSameShapes) {
    ...
    {
        // zero points
        const auto zp_const = std::make_shared<op::v0::Constant>(element::i8, ov::Shape{64, 1, 1, 1}, 1);
        const auto zp_convert = std::make_shared<op::v0::Convert>(zp_const, element::f32);

        const auto multiply_left = make_branch_same(zp_convert);
        const auto result_left = std::make_shared<op::v0::Result>(multiply_left);

        const auto multiply_right = make_branch_same(zp_convert);
        const auto result_right = std::make_shared<op::v0::Result>(multiply_right);
...
TEST_F(TransformationTestsF, MarkDequantizationScaleOnTheLeftBranch) {
    {
        auto lp_const = std::make_shared<op::v0::Constant>(element::u4, Shape{27}, 1);
        auto scale = op::v0::Constant::create(element::i64, Shape{}, {2});

        auto convert_lp = std::make_shared<op::v0::Convert>(lp_const, element::f32);
        auto convert_scale = std::make_shared<op::v0::Convert>(scale, element::f32);
        auto multiply = std::make_shared<op::v1::Multiply>(convert_scale, convert_lp);
        auto stub_op = std::make_shared<op::v0::Relu>(multiply);
    ...
    {
        auto lp_const = std::make_shared<op::v0::Constant>(element::u4, Shape{27}, 1);
        auto scale = op::v0::Constant::create(element::f32, Shape{}, {2});

        auto convert_lp = std::make_shared<op::v0::Convert>(lp_const, element::f32);
        auto multiply = std::make_shared<op::v1::Multiply>(scale, convert_lp);
        auto stub_op = std::make_shared<op::v0::Relu>(multiply);
...
TEST_F(TransformationTestsF, KeepConstPrecision) {
    {
        ...
        const auto target_shape = std::make_shared<op::v0::Constant>(ov::element::i64, ov::Shape{3}, 3);
        auto reshape = std::make_shared<op::v1::Reshape>(lp_const, target_shape, false);

        auto second_convert = std::make_shared<op::v0::Convert>(reshape, element::f32);
        auto zero_point = op::v0::Constant::create(element::f32, Shape{}, {127});
        auto subtract = std::make_shared<op::v1::Subtract>(second_convert, zero_point);
        auto scale = op::v0::Constant::create(element::f32, Shape{}, {0.2});
    ...
    {
        auto lp_const = std::make_shared<op::v0::Constant>(element::u4, Shape{3, 3, 3}, 1);
        auto second_convert = std::make_shared<op::v0::Convert>(lp_const, element::f32);
        auto zero_point = op::v0::Constant::create(element::f32, Shape{}, {127});
        auto subtract = std::make_shared<op::v1::Subtract>(second_convert, zero_point);
        auto scale = op::v0::Constant::create(element::f32, Shape{}, {0.2});
        auto multiply = std::make_shared<op::v1::Multiply>(subtract, scale);
...
TEST_F(TransformationTestsF, MarkDequantizationTransformation) {
    ...
    {
        auto parameter = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 16, 14, 14});
        std::shared_ptr<Node> activations =
            std::make_shared<op::v0::FakeQuantize>(parameter,
                                                    op::v0::Constant::create(element::f32, Shape{}, {0}),
                                                    op::v0::Constant::create(element::f32, Shape{}, {20}),
                                                    op::v0::Constant::create(element::f32, Shape{}, {0}),
                                                    op::v0::Constant::create(element::f32, Shape{}, {254}),
                                                    255);
        {
            auto first_convert = std::make_shared<op::v0::Convert>(activations, element::u8);
            auto second_convert = std::make_shared<op::v0::Convert>(first_convert, element::f32);
            auto zero_point = op::v0::Constant::create(element::u8, Shape{}, {127});
            auto convert_on_zero_point = std::make_shared<op::v0::Convert>(zero_point, element::f32);
            auto subtract = std::make_shared<op::v1::Subtract>(second_convert, convert_on_zero_point);
            auto scale = op::v0::Constant::create(element::f32, Shape{}, {0.2});
            auto multiply = std::make_shared<op::v1::Multiply>(subtract, scale);
            activations = multiply;
        }

        std::shared_ptr<Node> weights = op::v0::Constant::create(element::i8, Shape{4, 16, 1, 1}, {-2});
        {
            auto convert = std::make_shared<op::v0::Convert>(weights, element::f32);
            auto zero_point = op::v0::Constant::create(element::i8, Shape{}, {127});
            auto convert_on_zero_point = std::make_shared<op::v0::Convert>(zero_point, element::f32);
            auto subtract = std::make_shared<op::v1::Subtract>(convert, convert_on_zero_point);
            auto scale = op::v0::Constant::create(element::f32, Shape{}, {0.2});
            auto multiply = std::make_shared<op::v1::Multiply>(subtract, scale);
            weights = multiply;
        }

        auto conv = std::make_shared<op::v1::Convolution>(activations,
                                                           weights,
                                                           Strides{1, 1},
                                                           CoordinateDiff{0, 0},
                                                           CoordinateDiff{0, 0},
...
TEST_F(TransformationTestsF, MarkDequantizationTransformationNoZeroPoint) {
...

    {
                                                    ...
                                                    255);
        {
            ...
            auto second_convert = std::make_shared<op::v0::Convert>(first_convert, element::f32);
            auto scale = op::v0::Constant::create(element::f32, Shape{}, {0.2});
            auto multiply = std::make_shared<op::v1::Multiply>(second_convert, scale);
            activations = multiply;
        }
...
TEST_F(TransformationTestsF, MarkDequantizationTransformationNoZeroPointFP16) {
...

    {
                                                    ...
                                                    255);
        {
            auto first_convert = std::make_shared<op::v0::Convert>(activations, element::u8);
            auto second_convert = std::make_shared<op::v0::Convert>(first_convert, element::f32);
            auto scale = op::v0::Constant::create(element::f16, Shape{}, {0.2});
            auto scale_convert = std::make_shared<op::v0::Convert>(scale, element::f32);
            mark_as_decompression(scale_convert);
            auto multiply = std::make_shared<op::v1::Multiply>(second_convert, scale_convert);
            activations = multiply;
        }
        ...
        std::shared_ptr<Node> weights = op::v0::Constant::create(element::i8, Shape{4, 16, 1, 1}, {-2});
        {
            ...
            auto scale_convert = std::make_shared<op::v0::Convert>(scale, element::f32);
            mark_as_decompression(scale_convert);
            auto multiply = std::make_shared<op::v1::Multiply>(convert, scale_convert);
            weights = multiply;
        }
...
TEST_F(TransformationTestsF, MarkDequantizationTransformationNotConstantWeights) {
...

    {
        ...
        std::shared_ptr<Node> weights = op::v0::Constant::create(element::i8, Shape{4, 16, 1, 1}, {-3});
        enable_keep_const_precision(weights);
        {
            auto clamp = std::make_shared<op::v0::Clamp>(weights, -2, 2);
            auto convert = std::make_shared<op::v0::Convert>(clamp, element::f32);
            auto zero_point = op::v0::Constant::create(element::i8, Shape{}, {127});
            enable_keep_const_precision(zero_point);
            auto convert_on_zero_point = std::make_shared<op::v0::Convert>(zero_point, element::f32);
            auto subtract = std::make_shared<op::v1::Subtract>(convert, convert_on_zero_point);
...
TEST_F(TransformationTestsF, MarkDequantizationTransformationFoldSubConst) {
    ...
    {
        auto weights = op::v0::Constant::create(element::u8, Shape{4, 16, 1, 1}, {3});
        auto convert = std::make_shared<op::v0::Convert>(weights, element::f32);
        auto zero_point = op::v0::Constant::create(element::u8, Shape{}, {127});
        auto convert_on_zero_point = std::make_shared<op::v0::Convert>(zero_point, element::f32);
        auto subtract = std::make_shared<op::v1::Subtract>(convert, convert_on_zero_point);
...

    {
        ...
        pass::disable_constant_folding(convert);
        auto zero_point = op::v0::Constant::create(element::f32, Shape{}, {127});
        auto subtract = std::make_shared<op::v1::Subtract>(convert, zero_point);
        mark_as_dequantization_node(subtract);
        auto scale = op::v0::Constant::create(element::f32, Shape{}, {0.2});
        auto multiply = std::make_shared<op::v1::Multiply>(subtract, scale);
        mark_as_dequantization_node(multiply);
