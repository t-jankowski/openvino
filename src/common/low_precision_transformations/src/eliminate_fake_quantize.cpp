...
EliminateFakeQuantizeTransformation::EliminateFakeQuantizeTransformation(const Params& params) : CleanupTransformation(params) {
    ...
    const auto matcher = pattern::wrap_type<ov::op::v0::FakeQuantize>({
            pattern::any_input(),
...
namespace {
bool check_interval(const std::shared_ptr<ov::op::v0::FakeQuantize>& fq,
    ...
    if (need_to_check_intervals) {
        auto tmp_fq = as_type_ptr<ov::op::v0::FakeQuantize>(fq->clone_with_new_inputs({
            constant,
        ...
        const auto result_constant = as_type_ptr<ov::op::v0::Constant>(result);
...
bool check_intervals(const std::shared_ptr<ov::op::v0::FakeQuantize>& fakeQuantize) {
    ...
    return
        check_interval(fakeQuantize, ov::as_type_ptr<ov::op::v0::Constant>(fakeQuantize->get_input_node_shared_ptr(1)),
                       min_value, max_diff, exact_comparison) &&
        check_interval(fakeQuantize, ov::as_type_ptr<ov::op::v0::Constant>(fakeQuantize->get_input_node_shared_ptr(2)),
                       max_value, max_diff, exact_comparison) &&
        check_interval(fakeQuantize, ov::as_type_ptr<ov::op::v0::Constant>(fakeQuantize->get_input_node_shared_ptr(3)),
                       min_value, max_diff, true) &&
        check_interval(fakeQuantize, ov::as_type_ptr<ov::op::v0::Constant>(fakeQuantize->get_input_node_shared_ptr(4)),
...
bool EliminateFakeQuantizeTransformation::canBeTransformed(const std::shared_ptr<Node>& operation) const {
    ...
    const auto fakeQuantize = ov::as_type_ptr<ov::op::v0::FakeQuantize>(operation);
