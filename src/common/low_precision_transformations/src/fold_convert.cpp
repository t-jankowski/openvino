...
FoldConvertTransformation::FoldConvertTransformation(const Params& params) : CleanupTransformation(params) {
    ...
    auto subtract = pattern::wrap_type<ov::op::v1::Subtract>();
...
bool FoldConvertTransformation::transform(ov::pass::pattern::Matcher &m) {
    ...
    auto foldConvert = [&](const size_t branch) {
        ...
        if (!ov::is_type<ov::op::v0::Convert>(convert) || !ov::is_type<ov::op::v0::Constant>(convert->get_input_node_shared_ptr(0))) {
            return;
        }

        const auto resultConstant = ov::pass::low_precision::foldConvert(convert->input_value(0), convert->get_output_element_type(0));
        assert(ov::is_type<ov::op::v0::Constant>(resultConstant));
...
bool FoldConvertTransformation::canBeTransformed(const std::shared_ptr<Node>& operation) const {
    return
        ...
        ((ov::is_type<ov::op::v0::Convert>(operation->get_input_node_ptr(1)) &&
        ov::is_type<ov::op::v0::Constant>(operation->get_input_node_ptr(1)->get_input_node_ptr(0))) ||
        (ov::is_type<ov::op::v0::Convert>(operation->get_input_node_ptr(0)) &&
        ov::is_type<ov::op::v0::Constant>(operation->get_input_node_ptr(0)->get_input_node_ptr(0))));
}
