...
bool QuantizationDetails::outputLayoutIsSupported(std::shared_ptr<ov::op::v0::FakeQuantize> quantize, bool isConvertExpected) {
    const auto inputs = quantize->inputs();
    for (size_t i = 1; i < inputs.size(); ++i) {
        const auto node = inputs[i].get_source_output().get_node_shared_ptr();
        bool supported = ov::is_type<ov::op::v0::Constant>(node);
        if (!supported && isConvertExpected) {
            supported = ov::is_type<ov::op::v0::Convert>(node) && ov::is_type<ov::op::v0::Constant>(node->get_input_node_ptr(0));
        }
        if (!supported) {
            return false;
        }
...
void QuantizationDetails::getInputIntervals(
        std::shared_ptr<ov::op::v0::FakeQuantize> quantize,
        std::vector<float>& inputLowValues,
        std::vector<float>& inputHighValues) {
    std::shared_ptr<ov::op::v0::Constant> inputLowLayer = ov::as_type_ptr<ov::op::v0::Constant>(quantize->get_input_node_shared_ptr(1));
    ...
    std::shared_ptr<ov::op::v0::Constant> inputHighLayer = ov::as_type_ptr<ov::op::v0::Constant>(quantize->get_input_node_shared_ptr(2));
...
void QuantizationDetails::getOutputIntervals(
    ...
    std::shared_ptr<ov::op::v0::Constant> outputLowLayer = ov::as_type_ptr<ov::op::v0::Constant>(quantize->get_input_node_shared_ptr(3));
    ...
    std::shared_ptr<ov::op::v0::Constant> outputHighLayer = ov::as_type_ptr<ov::op::v0::Constant>(quantize->get_input_node_shared_ptr(4));
...
QuantizationDetails QuantizationDetails::getDetails(std::shared_ptr<ov::op::v0::FakeQuantize> quantize) {
    if (!QuantizationDetails::outputLayoutIsSupported(quantize)) {
        return QuantizationDetails();
    }

    const std::vector<float> inputLowValues = ov::as_type_ptr<ov::op::v0::Constant>(quantize->get_input_node_shared_ptr(1))->cast_vector<float>();
    const std::vector<float> inputHighValues = ov::as_type_ptr<ov::op::v0::Constant>(quantize->get_input_node_shared_ptr(2))->cast_vector<float>();

    const std::vector<float> outputLowValues = ov::as_type_ptr<ov::op::v0::Constant>(quantize->get_input_node_shared_ptr(3))->cast_vector<float>();
    const std::vector<float> outputHighValues = ov::as_type_ptr<ov::op::v0::Constant>(quantize->get_input_node_shared_ptr(4))->cast_vector<float>();
...
std::vector<float> QuantizationDetails::getBlobValue(std::shared_ptr<Node> constantLayer) {
    return ov::as_type_ptr<ov::op::v0::Constant>(constantLayer)->cast_vector<float>();
}
