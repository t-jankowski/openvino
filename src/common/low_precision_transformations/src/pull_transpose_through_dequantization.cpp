...
std::shared_ptr<Node> moveThroughElementwise(const std::shared_ptr<Node>& transpose, const std::shared_ptr<Node>& elementwise) {
    ...
    auto elementwiseValuesConvert = ov::as_type_ptr<op::v0::Convert>(elementwise->get_input_node_shared_ptr(1ul));
    ...
    assert(ov::is_type<op::v0::Constant>(elementwiseValues));
    ...
    if (elementwiseValuesShape.size() != shape_size(transposeValuesShape)) {
        ...
        elementwiseValues = ov::pass::low_precision::fold<op::v1::Broadcast>(
            elementwiseValues,
    ...
    const std::shared_ptr<op::v1::Transpose> newTranspose = ov::as_type_ptr<op::v1::Transpose>(transpose->clone_with_new_inputs({
        elementwise->get_input_node_shared_ptr(0ul),
        transposeValues }));

    const auto newElementwiseValues = ov::pass::low_precision::fold<op::v1::Transpose>(
        elementwiseValues,
        transposeValues);
...
void fuseConstant(const std::shared_ptr<Node>& transpose, const std::shared_ptr<Node>& constant) {
    const auto newConstant = ov::pass::low_precision::fold<op::v1::Transpose>(
        constant,
...
ov::pass::low_precision::PullTransposeThroughDequantization::PullTransposeThroughDequantization(
    ...
    const auto transposeConstant = ov::pass::pattern::wrap_type<ov::op::v0::Constant>();
    auto matcherTranspose = ov::pass::pattern::wrap_type<op::v1::Transpose>({ multiply, transposeConstant });
    ...
    ov::matcher_pass_callback callback = [=](ov::pass::pattern::Matcher & m) -> bool {
        ...
        while (transpose != nullptr) {
            ...
            if (ov::is_type<op::v1::Multiply>(parent) || ov::is_type<op::v1::Subtract>(parent)) {
                ...
                pull_transpose_through_dequantization::fuseConstant(transpose, ov::as_type_ptr<op::v0::Constant>(parent));
                transpose = nullptr;
            } else {
